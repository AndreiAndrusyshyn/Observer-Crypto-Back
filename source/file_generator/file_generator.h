...
public:
    ...
    void add_stack();
private:
    uint64_t block_number = 5, get_block;
    uint16_t user_amount = 4, get_user_amount;
    ...
    byte *buffer = new byte[sizeof(uint16_t) * sizeof(uint16_t) + sizeof(boost::uuids::uuid) + (sizeof(user_amount) * (sizeof(uint16_t)))];
};
